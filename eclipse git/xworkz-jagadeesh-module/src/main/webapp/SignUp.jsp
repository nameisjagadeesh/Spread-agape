...
<body>
	...
	<c:forEach items="${error}" var="e">${e.message} <br></c:forEach>
	</h5>
	...
	<form action="modules" method="post" >
		    ...
		    Phone Number<input type="number" placeholder="enter your number" name="number" id="userMobile" onchange="ValidMobile()" />
		     <span id="mobileError" style="color: red"></span><br> 
		     <span id="numberPrint" style="color: red;"></span><br>
			Email Id<input type="email"	placeholder="enter your emailId" name="email" id="userId" onchange="valideEmail()"/>  <span
			id="emailError" style="color: red"></span>	<br>
			<span id="emailPrint" style="color: red;"></span> 
			
			Password<input type="password" placeholder="enter your password"	name="password" id="userPassword" />
			<span id="passwordError"
			style="color: red"></span> <input type="checkbox"
			onclick="onPassword()">Show Password <br>	
			Confirm Password <input type="password"	placeholder="Confirm your Password" name="confirmPassword" id="userConfirmPassword" onchange=""/>
			<span	id="passwordCompare" style="color: red"></span><br> 
			Agreement <input type="checkbox" name="agreement" id="agreementConfirm" onclick="ValidName()" /> <br> 
			SignUp <input type="submit" value="SignUp" class="btn btn-primary" id="submitId"  disabled="true"/>
...
if(uservalue!=null && uservalue!="" && uservalue.length>2 && uservalue.length<25 )
{
	...
	console.log(uservalue);
	xhttp.open("GET","http://localhost:8080/xworkz-jagadeesh-module/userId/"+uservalue);
	xhttp.send();
...
function validEmail() {
	...
	const xhttp=new XMLHttpRequest();
	console.log(userEmailvalue);
	xhttp.open("GET", "http://localhost:8080/xworkz-jagadeesh-module/userEmail/"+userEmailvalue)
	xhttp.send();
	xhttp.onload=function(){
		console.log(this);
...
function ValidMobile() {
	...
	const xhttp=new XMLHttprequest();
	console.log(userMobilevalue);
	xhttp.open("GET","http://localhost:8080/xworkz-jagadeesh-module/userNumber/"+userMobilevalue);
	xhttp.send();
	xhttp.onload=function(){
		console.log(this);
...
function ValidPassword() {
	...
	<footer>
		<small>© 2023 Jagadeesh Allagi.</small>
	</footer>
